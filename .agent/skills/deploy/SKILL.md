...
**Имя скрипта = `release`, не `deploy`.** pnpm 9+ резервирует `deploy`
как встроенную команду (для workspace deploy), которая перехватывает
npm-script с тем же именем — `pnpm release` возвращает
`ERR_PNPM_CANNOT_DEPLOY  A deploy is only possible from inside a
workspace`. Поэтому `package.json` использует `release` / `release:preview`.
...
1. **`./dist` не существует** — `wrangler pages deploy` упадёт.
   `pnpm release` это решает (`pnpm build` встроен в скрипт), но
...
10. **`commit_message` UTF-8 false-positive** (wrangler 4.90.0,
    ...
    `pnpm release` передаёт `--commit-message "$(git log -1 --pretty=%s)"` —
