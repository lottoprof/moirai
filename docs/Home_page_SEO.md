...
#### Правило: 6 buying-blockers на home, остальное на /faq

На главной — **только 6 вопросов** (не "часто задаваемые", а те, которые
**мешают нажать Apply прямо сейчас**). Это разные множества: общая FAQ
живёт на `/{locale}/faq`, главная держит conversion-critical вопросы.

Обязательные категории (6 вопросов на home, фиксированный порядок):

1. **Online / время** — снимает первый барьер ("смогу ли я по времени?")
2. **Опыт / порог входа** — критично для complete beginners (largest segment)
3. **Оборудование** — снимает финансовый барьер ("у меня нет камеры")
4. **Время в неделю** — главный вопрос работающих взрослых
5. **Финальный результат** — превращает абстракт "пройти" в "у меня будет фильм"
6. **Когда старт** — urgency, переход к /apply

Опциональные (+1–2 если SEO/CTA-критично):
- Возврат денег — снимает финансовый риск
- Сертификат — только если действительно даём

**НЕ держать на главной:**
- Вопросы про различия тиров → /pricing или programme page
- Вопросы про инструкторов → /instructors/<slug>
- Tech (Zoom, доступ) → /faq full
- Checkout/валюта/налоги → во время checkout
- Корпоративные/B2B → отдельный flow

#### Schema.org FAQPage — split между home и /faq

Чтобы избежать **duplicate structured data** (Google понижает приоритет):

- **Home:** FaqSchema индексирует **первые 6** (`allFaqs.slice(0, 6)`) — buying blockers
- **/faq:** UI показывает ВСЕ вопросы; FaqSchema индексирует **только detailed**
  (`allFaqs.slice(6)`) — расширенные, не пересекающиеся с home

Source of truth — `home.faqs[]` (один data-источник). Index 0-5 = home,
6+ = /faq. Порядок в `home.{en,ru}.mdx` фиксирован: buying-blockers
сверху, разделитель-комментарий `# ── Detailed (7+, только на /faq) ──`.
