...
#### Сколько вопросов: 6–8

6–8 — диапазон, который Google рекомендует для FAQ-блоков и который
оптимален по UX.

**Меньше 5** — Schema.org FAQPage может не пройти Rich Results
(Google любит "содержательные" FAQ, 3 вопроса выглядят как формальность,
snippet не выдаётся).

**Больше 8–10** — на главной начинает забирать внимание от CTA. Главная
— это верхушка воронки, FAQ здесь снимает **первые блокеры покупки**,
а не отвечает на всё. Глубокие операционные вопросы — на детальных
страницах и в `/{locale}/faq`.

#### Какие вопросы держать на главной (приоритет — buying blockers)

Не "часто задаваемые", а те, **которые мешают нажать Apply прямо сейчас**.
Это разные множества.

**Обязательные категории (6 вопросов на home, фиксированный порядок):**

1. **Online / время** — "Полностью онлайн? Когда сессии?"
   Снимает первый барьер: "а смогу ли я по времени?"
2. **Опыт / порог входа** — "Нужен ли опыт?"
   Решающий вопрос для complete beginners сегмента — самой большой
   аудитории.
3. **Оборудование** — "Какая камера нужна?"
   Снимает финансовый барьер ("я не могу купить камеру за \$2000").
4. **Время в неделю** — "Сколько часов в неделю?"
   Совмещение с работой/учёбой — главный практический вопрос работающих
   взрослых.
5. **Финальный результат** — "Что я получу в конце?"
   Превращает абстракт "пройти программу" в конкретное "у меня будет
   фильм".
6. **Когда старт** — "Когда следующий поток?"
   Создаёт urgency, переводит на `/apply`.

**Опциональные (+1–2 если есть место):**
- **Возврат денег** — "Можно вернуть деньги?" — снимает финансовый риск.
- **Сертификат / диплом** — "Дадите сертификат?" — только если
  действительно даёте. Если нет — лучше не упоминать.

#### Что НЕ держать на главной

- Вопросы про конкретные тиры и их различия → на странице программы
  или `/pricing`
- Вопросы про инструкторов → на их профилях `/instructors/<slug>`
- Технические вопросы про Zoom/доступ → на `/faq` полном
- Вопросы про checkout, налоги, валюту → во время чекаута
- Корпоративные/групповые покупки → отдельный flow

#### Привязка к Schema.org FAQPage

Все 6 вопросов на главной → в Schema.org FAQPage JSON-LD. Этого
достаточно для Rich Results — Google часто показывает 4–6 вопросов из
блока в SERP. Больше 10 — отбраковывает излишек.

**Важно:** Schema.org FAQPage должен быть **только на главной** (или
на отдельной `/faq` странице, но **не на обеих с одинаковыми вопросами**
— это duplicate structured data, понижение приоритета).

#### Стратегия покрытия SERP (3 страницы, разные вопросы)

- **Главная** — 6–8 buying blocker вопросов с Schema.org FAQPage
- **`/{locale}/faq`** — полный список 20–30 вопросов разбитый по
  категориям, тоже с FAQPage Schema, но **другие вопросы** (без дублей
  с главной)
- **Страницы программ** `/programmes/<slug>` — 3–5 specific вопросов в
  собственном FAQPage Schema блоке

Так покрываем больше SERP-real-estate без каннибализации.

#### Текущая реализация (single data source)

Source of truth — `home.faqs[]` в `src/content/pages/home.{en,ru}.mdx`.
Один data-источник, разделение между home и /faq:

- **Home:** FaqSchema индексирует **первые 6** (`allFaqs.slice(0, 6)`) —
  buying blockers
- **/faq:** UI показывает ВСЕ вопросы; FaqSchema индексирует **только
  detailed** (`allFaqs.slice(6)`) — расширенные, не пересекающиеся с
  home

Порядок в `home.{en,ru}.mdx` фиксирован: buying-blockers сверху,
разделитель-комментарий `# ── Detailed (7+, только на /faq) ──`.

#### Правило при появлении нового вопроса

**НЕ добавлять "ещё один важный вопрос" на главную автоматически.**
Сначала проверить: это buying blocker (мешает нажать Apply сейчас)?

- Да → заменить один из 6 существующих, если он стал менее критичным
- Нет → добавить в detailed-секцию `home.faqs[]` (index 6+)

Это снимает будущее искушение "добавим ещё один важный вопрос" и
сохраняет инвариант 6 buying-blockers на home.
